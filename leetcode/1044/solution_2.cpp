...
Runtime: 29 ms, faster than 99.03% of C++ online submissions for Longest Duplicate Substring.
Memory Usage: 14.6 MB, less than 88.65% of C++ online submissions for Longest Duplicate Substring.
...
public:
    SuffixArray(const string& s, size_type n, size_type m)
        ...
        , lcp(rk + n)
    {
        ...
        sais(s, sa, type.get(), rk, lcp, lbuk.get(), sbuk.get(), n, m);
        for (size_type i = 0; i < n; ++i)
            rk[sa[i]] = i;
    ...
    size_type heiglcp(size_type p) const
    {
        return lcp[p];
    }
};
...
public:
    string longestDupSubstring(const string& s)
    {
        ...
        int max_len = 0, pos = n;
        for (int i = 1; i < n; ++i) {
            if (const auto len = sa.lcp[i]; len > max_len) {
                max_len = len;
                pos = sa.sa[i];
            }
        }
