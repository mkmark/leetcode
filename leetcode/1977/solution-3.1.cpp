...
Runtime: 959 ms, faster than 38.89% of C++ online submissions for Number of Ways to Separate Numbers.
Memory Usage: 183.3 MB, less than 11.11% of C++ online submissions for Number of Ways to Separate Numbers.

dfs + dp + sum dp + suffix array
use 1d lcp observing that operator[] takes significant amount of time
*/
...
public:
    ...
    int numberOfCombinations(string& num) {
        ...
        rk = suffixArray.rk;
        vector<unsigned> lcp(n*n, UINT32_MAX);
        ptr_lcp = & lcp;
        for (int i = 0; i < n; ++i){
            for (int j = i+1; j < n; ++j){
                lcp[i*n+j] = min(lcp[i*n+j-1], suffixArray.ht[j]);
            }
        }
    ...
    inline int get_min_nxt_end(int& l, int& r){
        ...
        if (rk[nxt_l] < rk[l] && ((*ptr_lcp)[rk[nxt_l]*n+rk[l]] <= r-l)){
            return r*2+2-l;
        } else {
            return r*2+1-l;
        }
    ...
    int dfs_comb(int l, int r){
        ...
        if (min_nxt_end){
            ...
            dp[l][r] = (*ptr_dpsum)[r+1][min_nxt_end];
            dp[l][r] %= 1000000007;
            return dp[l][r];
        } else {
            return dp[l][r] = 0;
